...
function wkStatus(wn){
  // Count completed days for this week from state keys
  ...
  // A week with content has 6 days; consider done if complement + at least 5 days
...
function renderIndex(){
  ...
  WEEKS_INDEX.forEach(w=>{
    ...
    html+='<div class="wc '+(w.hasContent?'hc ':'')+sc+'" onclick="goWeek('+w.num+')">'
...
async function initApp(){
  ...
  // Session found — load data and show index
...
// Attach login handlers after DOM ready
